# Cinnamon session settings (fallback DE next to hyprland). Terminal goes
# through handlr so the default terminal is shared with the wayland session.
{self, ...}: {
  flake.homeModules.homeCinnamon = {lib, ...}: {
    imports = [
