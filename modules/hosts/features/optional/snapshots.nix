...
# creates and mounts the /snapshots/{root,persist} subvolumes (kept outside the
# rolled-back root subvolume) before snapper needs them. Requires the
# diskoBtrfs disk layout.
{
      ...
      script = ''
        ...
        mount_if_needed() {
          ...
          if findmnt -rno SOURCE,TARGET "$target" >/dev/null 2>&1; then
            ...
            mount -t btrfs -o "subvol=$subvol,compress=zstd" ${partition} "$target"
          fi
        }
